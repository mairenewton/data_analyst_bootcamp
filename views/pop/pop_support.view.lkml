#Reference Article: https://community.looker.com/lookml-5/use-refinements-to-add-period-over-period-functionality-to-existing-explores-18652
#See view file Views/Marketing_Data/advertisment_platofrms_summary.view for the refinement needed in the explore
view: pop_support {
  derived_table: {
    sql:
  ...
  dimension: pop_sql_years_using_now  {type: date_raw expression:  add_years(${periods_ago},${now_sql});;}
  dimension: pop_sql_quarters_using_now {type: date_raw sql: DATEADD(quarter,${periods_ago},${now_sql});; }
  dimension: pop_sql_months_using_now {type: date_raw expression:  add_months(${periods_ago},${now_sql});;}
  dimension: pop_sql_weeks_using_now  {type: date_raw sql: DATEADD(week,${periods_ago},${now_sql});;}
